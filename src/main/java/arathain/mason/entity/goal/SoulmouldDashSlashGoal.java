...
public class SoulmouldDashSlashGoal extends Goal {
    ...
    public void tick() {
        ...
        if(ticks == 10 || ticks == 13 || ticks == 15) {
            ...
            List<LivingEntity> entities = mould.getWorld().getEntitiesByClass(LivingEntity.class, mould.getBoundingBox().expand(4, 3, 4), livingEntity -> livingEntity != mould && livingEntity != mould.getOwner() && !(livingEntity instanceof SoulmouldEntity smould && smould.getOwner() == mould.getOwner()) && !(livingEntity instanceof AbstractMannequin) && (livingEntity instanceof AnimatedStatueEntity statue && statue.getOwnerUuid().equals(mould.getOwnerUuid()) || !(livingEntity instanceof AnimatedStatueEntity)) && mould.distanceTo(livingEntity) <= 4 + livingEntity.getWidth() / 2 && livingEntity.getY() <= mould.getY() + 3);
